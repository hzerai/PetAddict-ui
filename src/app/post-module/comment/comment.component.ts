...
export class CommentComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.imageService.getImage('USER-' + this.comment.userId).subscribe
      (i => {
        if (i == null) { this.comment.image = 'https://www.w3schools.com/howto/img_avatar.png' }
        else { this.comment.image = i.bytes }
      });
  ...
  repcomment() {
    if (this.commentBody.length > 0) {
      ...
      comment.comments = [];
      this.postService.reply(this.post.id, this.comment.id, comment).subscribe(c => {
        this.comment.comments.unshift(c);
      });
      this.commentBody = "";
